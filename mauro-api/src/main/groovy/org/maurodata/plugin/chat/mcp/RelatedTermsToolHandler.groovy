...
@McpToolDefinition(
    ...
    useWhen = [
        ...
        'finding related terms as part of a catalogue search task; after this tool returns, call mauro_search next unless the request explicitly asks only for suggestions and no search'
    ],
    avoidWhen = [
...
class RelatedTermsToolHandler extends AbstractAnnotatedToolHandler {
    ...
    String modelText(Map<String, Object> result) {
        ...
        renderModelTextSections([
            ...
            'Answer Instructions': [
                ...
                'If the user asked for terms as part of a search task, or framed this as query expansion for a catalogue search, use these terms to call mauro_search next rather than only telling the user to search themselves.',
                ...
                'If the user later gives a brief confirmation such as "ok", "yes", "sure", "please", or "go ahead", treat that as permission to run mauro_search with the Suggested Search Expression.',
                'When the user explicitly asks to use related terms in a search task, carry the looked-up related terms into mauro_search; do not replace them with a narrower single-term search.',
                'When calling mauro_search after this tool, use the Suggested Search Expression as the mauro_search searchTerm unless there is a clear tool-schema reason not to. Preserve any domainTypes implied by already-injected representation skills.',
            ...
            'Completion Guidance': [
                ...
                'If the user asked for related terms as part of a search task, this result is not the final answer; call mauro_search next using the Suggested Search Expression.',
    ...
    private static List<String> buildSearchExpression(Object inputText, List<?> terms) {
        ...
        ['Use this as a candidate mauro_search searchTerm: ' + values.take(8).join(' OR ')]
    }
