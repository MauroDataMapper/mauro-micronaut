...
CREATE OR REPLACE AGGREGATE search.tsvector_agg(tsvector) (
...
CREATE MATERIALIZED VIEW IF NOT EXISTS search.metadata_agg AS
SELECT
...
CREATE UNIQUE INDEX IF NOT EXISTS metadata_agg_pk
    ON search.metadata_agg (multi_facet_aware_item_id);

CREATE INDEX IF NOT EXISTS metadata_agg_ts_gin
    ON search.metadata_agg
    USING GIN (metadata_ts);


CREATE MATERIALIZED VIEW IF NOT EXISTS search.search_domains AS
SELECT
    sd.id,
    sd.domain_type,
    sd.label,
...
CREATE UNIQUE INDEX IF NOT EXISTS search_domains_pk
    ON search.search_domains (domain_type, id);

-- single GIN index for search
CREATE INDEX IF NOT EXISTS search_domains_combined_ts_gin
    ON search.search_domains
    USING GIN (combined_ts);

-- filters
CREATE INDEX IF NOT EXISTS search_domains_model_id_idx
    ON search.search_domains (model_id);

CREATE INDEX IF NOT EXISTS search_domains_domain_type_idx
    ON search.search_domains (domain_type);

CREATE INDEX IF NOT EXISTS search_domains_date_created_idx
    ON search.search_domains (date_created);

CREATE INDEX IF NOT EXISTS search_domains_last_updated_idx
    ON search.search_domains (last_updated);

CREATE TABLE IF NOT EXISTS search.search_domains_dirty_flag (
