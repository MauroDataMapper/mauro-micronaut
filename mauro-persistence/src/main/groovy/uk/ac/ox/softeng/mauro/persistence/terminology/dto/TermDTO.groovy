...
class TermDTO extends Term implements AdministeredItemDTO {
    ...
    @MappedProperty
    @ColumnTransformer(read = '(select json_agg(edit) from core.edit where multi_facet_aware_item_id = term_.id)')
    List<Edit> edits = []
    ...
    @ColumnTransformer(read = '''(select json_agg(rule) from (select *,
                                    ...
                                    from core.rule) rule where multi_facet_aware_item_id = term_.id)''')
    List<Rule> rules = []
    ...
    @ColumnTransformer(read = '''(select json_agg(annotation)  from (select *,
                                ...
                                from core.annotation) annotation where
                                                      multi_facet_aware_item_id = term_.id
