...
@CompileStatic
@Introspected
@MappedEntity(value = 'data_element_component', schema = 'dataflow', alias = 'data_element_component_')
class DataElementComponentDTO extends DataElementComponent implements AdministeredItemDTO {

    @Nullable
    @TypeDef(type = DataType.JSON)
    @MappedProperty
    @ColumnTransformer(read = '(select json_agg(metadata) from core.metadata where multi_facet_aware_item_id = data_element_component_.id)')
    ...
    @ColumnTransformer(read = '''(select json_agg(annotation)  from (select *,
                                ...
                                from core.annotation) annotation where
                                                      multi_facet_aware_item_id = data_element_component_.id
