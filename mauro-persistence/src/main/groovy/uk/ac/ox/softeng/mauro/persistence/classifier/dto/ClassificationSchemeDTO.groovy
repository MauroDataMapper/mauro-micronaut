package uk.ac.ox.softeng.mauro.persistence.classifier.dto
...
@CompileStatic
@Introspected
@MappedEntity(value = 'classification_scheme', schema = 'core', alias = 'classification_scheme_')
class ClassificationSchemeDTO extends ClassificationScheme implements AdministeredItemDTO {


    @Nullable
    @TypeDef(type = DataType.JSON)
    @MappedProperty
    @ColumnTransformer(read = '(select json_agg(metadata) from core.metadata where multi_facet_aware_item_id = classification_scheme_.id)')
    ...
    @ColumnTransformer(read = '''(select json_agg(summary_metadata) from (select *,
                                    ...
                                    from core.summary_metadata) summary_metadata where multi_facet_aware_item_id = classification_scheme_.id)''')
    ...
    @ColumnTransformer(read = '''(select json_agg(rule) from (select *,
                                    ...
                                    from core.rule) rule where multi_facet_aware_item_id = classification_scheme_.id)''')
    List<Rule> rules = []
    ...
    @ColumnTransformer(read = '''(select json_agg(annotation)  from (select *,
                                ...
                                from core.annotation) annotation where
                                                      multi_facet_aware_item_id = classification_scheme_.id
