...
class CodeSetDTO extends CodeSet implements AdministeredItemDTO {
    ...
    @ColumnTransformer(read = '''(select json_agg(summary_metadata) from (select *,
                                    ...
                                    from core.summary_metadata) summary_metadata where multi_facet_aware_item_id = code_set_.id)''')
    ...
    @ColumnTransformer(read = '''(select json_agg(rule) from (select *,
                                    ...
                                    from core.rule) rule where multi_facet_aware_item_id = code_set_.id)''')
    List<Rule> rules = []
