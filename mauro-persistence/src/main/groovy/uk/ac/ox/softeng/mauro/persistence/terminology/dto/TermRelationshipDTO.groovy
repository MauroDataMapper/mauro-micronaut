...
class TermRelationshipDTO extends TermRelationship implements AdministeredItemDTO {
    ...
    @TypeDef(type = DataType.JSON)
    @MappedProperty
    @ColumnTransformer(read = '''(select json_agg(to_jsonb(reference_file) - 'file_contents') from core.reference_file where multi_facet_aware_item_id = term_relationship_.id)''')
