...
class TermRelationshipTypeDTO extends TermRelationshipType implements AdministeredItemDTO {
    ...
    @MappedProperty
    @ColumnTransformer(read = '(select json_agg(edit) from core.edit where multi_facet_aware_item_id = term_relationship_type_.id)')
    List<Edit> edits = []
    ...
    @ColumnTransformer(read = '''(select json_agg(summary_metadata) from (select *,
                                    ...
                                    from core.summary_metadata) summary_metadata where multi_facet_aware_item_id = term_relationship_type_.id)''')
    ...
    @ColumnTransformer(read = '''(select json_agg(rule) from (select *,
                                    ...
                                    from core.rule) rule where multi_facet_aware_item_id = term_relationship_type_.id)''')
    List<Rule> rules = []
