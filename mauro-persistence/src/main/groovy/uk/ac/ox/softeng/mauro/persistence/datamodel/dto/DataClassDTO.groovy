...
class DataClassDTO extends DataClass implements AdministeredItemDTO {
    ...
    @MappedProperty
    @ColumnTransformer(read = '(select json_agg(metadata) from core.metadata where multi_facet_aware_item_id = data_class_.id)')
    ...
    @ColumnTransformer(read = '''(select json_agg(summary_metadata) from (select *,
                                    ...
                                    from core.summary_metadata) summary_metadata where multi_facet_aware_item_id = data_class_.id)''')
    ...
    @ColumnTransformer(read = '''(select json_agg(annotation)  from (select *,
                                ...
                                from core.annotation) annotation where
                                                      multi_facet_aware_item_id = data_class_.id
    ...
    @TypeDef(type = DataType.JSON)
    @MappedProperty
    @ColumnTransformer(read = '''(select json_agg(to_jsonb(reference_file) - 'file_contents') from core.reference_file where multi_facet_aware_item_id = data_class_.id)''')
