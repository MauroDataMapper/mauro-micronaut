...
class EnumerationValueDTO extends EnumerationValue implements AdministeredItemDTO {
    ...
    @ColumnTransformer(read = '''(select json_agg(annotation)  from (select *,
                                ...
                                from core.annotation) annotation where
                                                      multi_facet_aware_item_id = enumeration_value_.id
    ...
    @TypeDef(type = DataType.JSON)
    @MappedProperty
    @ColumnTransformer(read = '''(select json_agg(to_jsonb(reference_file) - 'file_contents') from core.reference_file where multi_facet_aware_item_id = enumeration_value_.id)''')
